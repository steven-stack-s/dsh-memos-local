 ...
 */
export function createDeepSeekHarnessHostLlmBridge(options) {
...
async function resolveAuxiliaryReasoningCapability(llm, route, signal, cache) {
    ...
    const value = llm.resolveModelInfo(route.provider, route.model, signal)
        .then((info) => (info.reasoning?.efforts.some((effort) => effort.id === NO_REASONING_EFFORT)
        ? "off"
        : "plain"))
        .catch((error) => {
...
//# sourceMappingURL=host-llm.js.map