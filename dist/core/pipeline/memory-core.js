...
import { makeRepos, embeddingMaintenanceCounts, inferStoredEmbeddingByteLen, FLOAT32_BYTES, } from "../storage/repos/index.js";
...
import { buildLocalRetrievalLogStages, } from "./retrieval-log.js";
    ...
    try {
        ...
        if (l3Model && l3Provider) {
            l3Llm = createLlmClient({
                ...
                maxRetries: 3,
                fallbackToHost: true,
                onError: (d) => log.warn("l3Llm.llm_error", d),
            });
...
export function createMemoryCore(handle, home, pkgVersion, options = {}) {
    ...
    const startupRecoveryShutdownGraceMs = Math.max(0, options.startupRecoveryShutdownGraceMs ?? 15_000);
    ...
    async function searchHubMemoryHits(query, limit = 5, deadlineAt) {
        ...
        try {
            const hits = await withTimeout(hubRuntime.searchMemories(query, limit), timeoutMs, "hub_search_timeout");
    ...
    async function finalFilterMergedHits(input) {
        ...
        let filtered = await llmFilterCandidates({
            ...
            timeoutMs: input.deadlineAt === undefined
                ...
                : Math.max(1, Math.min(DEADLINE_FILTER_SAFE_CUTOFF_MS, input.deadlineAt - Date.now())),
    ...
    async function recoverOpenEpisodesAsSessionEnd(orphans) {
        ...
        // behaviour.
        const captureFailedInBatch = new Set();
    ...
    async function recoverDirtyClosedEpisodes(episodes) {
        ...
        for (const ep of episodes) {
            ...
            const prevAttempts = typeof prevDirty.failedAttempts === "number"
    ...
    function episodeRewardIsDirty(ep) {
        ...
        if (typeof traceCount === "number") {
            ...
            const traceIds = (ep.traceIds ?? []);
            const existingCount = traceIds.length === 0
                ? 0
    ...
    function snapshotFromRecoveredEpisode(ep, endedAt, opts = {}) {
        ...
        for (const tr of traces) {
            const turnId = typeof tr.turnId === "number" && Number.isFinite(tr.turnId)
                ? tr.turnId
                : tr.ts;
            if (tr.userText) {
                turns.push({
    ...
    async function shutdown() {
        ...
        try {
            ...
            try {
                ...
                await withTimeout(startupRecoveryPromise, startupRecoveryShutdownGraceMs, "startup_recovery_shutdown_timeout");
            }
            catch (err) {
            ...
            await handle.shutdown("memory-core.shutdown", startupRecoveryTimedOut ? { flushGraceMs: 0 } : undefined);
        }
        finally {
    ...
    async function health() {
        ...
        const skillEvolverInfo = resolveSkillEvolver(effectiveConfig, 
        ...
        // in that case anyway.
        handle.reflectLlm ?? handle.llm, llmInfo, latestTraceTimestamp);
    ...
    async function searchMemory(query, execution = {}) {
        ...
        try {
            ...
            retrievalStats = s
                ? withHubStats(retrievalStatsPayload(s), hubCandidates.length, filtered.length, finalHubKept, finalFilterStats)
                : undefined;
    ...
    async function countSkills(input) {
        ensureLive();
        return handle.repos.skills.list({ status: input?.status, limit: 100_000 }).filter((r) => (input?.includeAllNamespaces || visibleToCurrent(r)) && matchesNamespaceFilter(r, input)).length;
    }
    ...
    async function metrics(input) {
        ...
        const totalTurns = handle.repos.traces.countTurns({}, input?.includeAllNamespaces ? undefined : visibilityWhere(activeNamespace));
        return {
            total: totalTurns,
            writesToday,
    ...
    function hasRewardDirtyMarker(ep) {
        return Boolean(ep.meta?.rewardDirty &&
