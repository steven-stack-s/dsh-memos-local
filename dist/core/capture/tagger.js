 ...
 */
export function tagsForStep(step) {
    const bag = new Set();
...
export function tagsForEpisode(steps) {
    const bag = new Set();
    for (const s of steps) {
        for (const t of tagsForStep(s))
            bag.add(t);
        if (bag.size >= MAX_TAGS * 2)
            break;
    }
