 ...
 */
export function tracePolicySimilarity(trace, policy, policySignature) {
    ...
    const polSig = policySignature ?? parseSignature(`${policy.trigger.slice(0, 1)}|_|_|_`); // fallback
    ...
    const bothDifferent = isDistinct(traceSig.primaryTag, polSig?.primaryTag) &&
 ...
 */
export function traceTraceSimilarity(a, b) {
 ...
 */
export function valueWeightedMean(traces, tau) {
    if (traces.length === 0)
        return 0;
 ...
 */
export function arithmeticMeanValue(traces) {
    if (traces.length === 0)
        return 0;
    let s = 0;
