...
export function openDb(opts) {
    ...
    const db = {
        ...
        prepare(sql) {
            ...
            const s = stmt;
            return {
                run: (args) => (args === undefined ? s.run() : s.run(args)),
                get: (args) => (args === undefined ? s.get() : s.get(args)),
                all: (args) => (args === undefined ? s.all() : s.all(args)),
                iterate: (args) => (args === undefined
                    ? s.iterate()
                    : s.iterate(args)),
            };
        },
