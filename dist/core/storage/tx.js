 ...
 */
export function buildInsert(spec) {
 ...
 */
export function buildUpdate(spec) {
    const pk = spec.pk ?? "id";
 ...
 */
export function chunkIn(items, chunkSize = 500) {
    if (items.length === 0)
        return [];
    const out = [];
...
export function buildInClause(n) {
    if (n <= 0)
...
// ─── Type guards ──────────────────────────────────────────────────────────────
export function isBetterSqliteError(err) {
