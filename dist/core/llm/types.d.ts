...
export type LlmProviderName = "local_only" | "openai_compatible" | "anthropic" | "gemini" | "bedrock" | "host";
...
export interface LlmProvider {
     ...
     */
    stream?(messages: LlmMessage[], opts: ProviderCallInput, ctx: LlmProviderCtx): AsyncIterable<LlmStreamChunk>;
    close?(): Promise<void>;
}
...
export interface LlmClient {
    ...
    complete(messages: LlmMessage[] | string, opts?: LlmCallOptions): Promise<LlmCompletion>;
    /**
     ...
     */
    completeJson<T>(messages: LlmMessage[] | string, opts?: LlmCompleteJsonOptions<T>): Promise<LlmJsonCompletion<T>>;
    /** Streaming text. Falls back to one-chunk emit if the provider lacks stream. */
    stream(messages: LlmMessage[] | string, opts?: LlmCallOptions): AsyncIterable<LlmStreamChunk>;
