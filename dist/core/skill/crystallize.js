...
import { sanitizeDerivedList, sanitizeDerivedMarkdown, sanitizeDerivedMarkdownList, sanitizeDerivedText, } from "../safety/content.js";
 ...
 */
export async function crystallizeDraft(input, deps) {
    ...
    const evidenceLang = detectDominantLanguage(evidenceSamples.some((sample) => sample?.trim())
        ? evidenceSamples
...
function prepareDraftFromResponse(rsp, input, log, attempt) {
    try {
...
function logDraftShape(prepared, rsp, input, log, attempt) {
    const common = {
        policyId: input.policy.id,
        provider: rsp.provider,
        model: rsp.model,
...
function validatePreparedDraft(draft, shape, rsp, validate, log, input, attempt) {
    try {
 ...
 */
function packPrompt(input, config) {
    ...
    if (repairHints.preference.length > 0 ||
 ...
 * enough to persist.
 */
function normaliseDraft(raw, input) {
    const rawName = String(raw.name ?? "").trim();
    const name = sanitiseName(rawName || `skill_${input.policy.id.slice(-6)}`);
    const displayTitle = sanitizeDerivedText(raw.display_title ?? raw.displayTitle ?? input.policy.title ?? name) ||
        name;
 ...
 */
export function defaultDraftValidator(draft) {
    if (!draft.name)
    ...
    if (!draft.summary) {
        ...
        // null/undefined.
        const autoSummary = draft.steps?.[0]?.body ||
            draft.steps?.[0]?.title ||
            draft.displayTitle ||
            draft.name;
