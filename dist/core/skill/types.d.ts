 ...
 */
import type { EpisodeId, PolicyRow, SkillId, SkillRow, TraceRow } from "../types.js";
/**
 ...
 */
export type SkillLifecycleTransition = "crystallized" | "verified" | "rejected" | "promoted" | "demoted" | "archived" | "rebuilt";
/**
 * Kind of user / runtime signal that drives an η update.
 */
export type SkillFeedbackKind = "trial.pass" | "trial.fail" | "user.positive" | "user.negative" | "reward.updated";
...
export type SkillEventKind = "skill.eligibility.checked" | "skill.crystallization.started" | "skill.crystallized" | "skill.verification.passed" | "skill.verification.failed" | "skill.status.changed" | "skill.eta.updated" | "skill.archived" | "skill.rebuilt" | "skill.failed";
export interface SkillEventBase<K extends SkillEventKind> {
    kind: K;
    at: number;
}
export interface SkillEligibilityCheckedEvent extends SkillEventBase<"skill.eligibility.checked"> {
...
export interface SkillCrystallizationStartedEvent extends SkillEventBase<"skill.crystallization.started"> {
    policyId: string;
    evidenceCount: number;
}
export interface SkillCrystallizedEvent extends SkillEventBase<"skill.crystallized"> {
    skillId: SkillId;
    name: string;
    policyId: string;
    status: SkillRow["status"];
}
export interface SkillVerificationPassedEvent extends SkillEventBase<"skill.verification.passed"> {
    skillId: SkillId;
    coverage: number;
}
...
export interface SkillStatusChangedEvent extends SkillEventBase<"skill.status.changed"> {
    skillId: SkillId;
    previous: SkillRow["status"];
    next: SkillRow["status"];
    transition: SkillLifecycleTransition;
}
export interface SkillEtaUpdatedEvent extends SkillEventBase<"skill.eta.updated"> {
    skillId: SkillId;
    previous: number;
    next: number;
...
export type SkillEvent = SkillEligibilityCheckedEvent | SkillCrystallizationStartedEvent | SkillCrystallizedEvent | SkillVerificationPassedEvent | SkillVerificationFailedEvent | SkillStatusChangedEvent | SkillEtaUpdatedEvent | SkillArchivedEvent | SkillRebuiltEvent | SkillFailedEvent;
...
export type SkillTrigger = "l2.policy.induced" | "l2.policy.status_changed" | "reward.updated" | "manual" | "rebuild" | "lifecycle.tick";
