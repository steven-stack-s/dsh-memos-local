...
import { crystallizeDraft, defaultDraftValidator, } from "./crystallize.js";
import { evaluateEligibility } from "./eligibility.js";
import { gatherEvidence } from "./evidence.js";
import { applyFeedback, recomputeEta, } from "./lifecycle.js";
import { buildSkillRow } from "./packager.js";
import { verifyDraft } from "./verifier.js";
export async function runSkill(input, deps) {
    ...
    for (const decision of eligibility.decisions) {
        ...
        const tVerify = nowMs();
        const verdict = verifyDraft({ draft: crystResult.draft, evidence: evidence.traces }, { log: log.child({ channel: "core.skill.verifier" }) });
 ...
 */
export function applySkillFeedback(skillId, kind, deps, magnitude) {
...
async function runCrystallize(policy, evidence, counterExamples, skillsByPolicy, deps) {
    ...
    // available.
    const triggerEpisodeId = policy.sourceEpisodeIds[policy.sourceEpisodeIds.length - 1] ??
