...
export class MistralEmbeddingProvider {
    ...
    async embed(texts, _role, ctx) {
        ...
        if (!config.apiKey) {
            throw new MemosError(ERROR_CODES.EMBEDDING_UNAVAILABLE, "mistral provider requires config.embedding.apiKey", { provider: this.name });
        }
        ...
        if (!Array.isArray(rows)) {
            throw new MemosError(ERROR_CODES.EMBEDDING_UNAVAILABLE, "mistral returned no data[]", { provider: this.name, url });
        }
        return rows.map((r, i) => {
            if (!Array.isArray(r.embedding)) {
                throw new MemosError(ERROR_CODES.EMBEDDING_UNAVAILABLE, `mistral row ${i} missing embedding`, { provider: this.name, url });
            }
            return r.embedding;
        });
    }
