...
export function registerSkillRoutes(routes, deps) {
    routes.set("GET /api/v1/skills", async (ctx) => {
        ...
        if (q) {
            all = all.filter((s) => s.name.toLowerCase().includes(q) || s.invocationGuide.toLowerCase().includes(q));
        }
    ...
    routes.set("GET /api/v1/skills/get", async (ctx) => {
        ...
        const skill = await deps.core.getSkill(id);
        if (skill === null) {
    ...
    routes.setPattern("GET /api/v1/skills/:id/timeline", async (ctx) => {
        ...
        const picks = await Promise.all(["skill_generate", "skill_evolve"].map((tool) => deps.core.listApiLogs({ toolName: tool, limit: 500, offset: 0 })));
        const entries = [];
        for (const res of picks) {
            for (const row of res.logs) {
                const out = safeJson(row.outputJson);
                const inp = safeJson(row.inputJson);
                const skillId = out?.skillId ??
                    inp?.skillId;
                if (skillId !== id)
                    continue;
                const kind = out?.kind ??
                    row.toolName;
                const phase = inp?.phase;
                const summary = out?.name ??
                    out?.reason;
                entries.push({
                    ts: row.calledAt,
                    kind,
    ...
    routes.setPattern("GET /api/v1/skills/:id/usage", async (ctx) => {
        ...
        const sourcePolicies = await Promise.all(skill.sourcePolicyIds.map(async (pid) => {
        ...
        const sourceWorldModels = await Promise.all(skill.sourceWorldModelIds.map(async (wid) => {
    ...
    routes.set("POST /api/v1/skills/archive", async (ctx) => {
        ...
        await deps.core.archiveSkill(target, body.reason);
        return { ok: true };
    });
    /**
    ...
    routes.set("POST /api/v1/skills/reactivate", async (ctx) => {
        ...
        const updated = await deps.core.reactivateSkill(target);
        if (!updated) {
    ...
    routes.setPattern("DELETE /api/v1/skills/:id", async (ctx) => {
        ...
        return await deps.core.deleteSkill(id);
    });
    /**
    ...
    routes.setPattern("PATCH /api/v1/skills/:id", async (ctx) => {
        ...
        const body = parseJson(ctx);
        const updated = await deps.core.updateSkill(id, {
            name: typeof body.name === "string" ? body.name : undefined,
            invocationGuide: typeof body.invocationGuide === "string" ? body.invocationGuide : undefined,
        });
        if (!updated) {
            writeError(ctx, 404, "not_found", `skill not found: ${id}`);
            return;
    ...
    routes.setPattern("POST /api/v1/skills/:id/share", async (ctx) => {
        ...
        const updated = await deps.core.shareSkill(id, {
            scope: scope ?? null,
...
function skillDescription(invocationGuide, name) {
    const fallback = `Use ${name} when this learned skill matches the current task.`;
    const lines = (invocationGuide ?? "")
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean);
    const firstBodyLine = lines.find((line) => !line.startsWith("#") && !line.startsWith(">")) ?? fallback;
