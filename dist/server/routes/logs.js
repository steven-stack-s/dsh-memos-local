...
export function registerLogsRoutes(routes, deps, options) {
    const tailSize = Math.max(10, Math.min(options.logTailSize ?? 200, 5_000));
    routes.set("GET /api/v1/logs/tail", async (ctx) => {
        const n = Math.min(Math.max(parseInt(ctx.url.searchParams.get("n") ?? String(tailSize), 10) || tailSize, 1), tailSize);
...
function handleLogStream(ctx, deps, tailSize) {
    const { res } = ctx;
    res.writeHead(200, {
