...
export function registerPoliciesRoutes(routes, deps) {
    // ─── Policies (L2 "经验") ────────────────────────────────────────
    ...
    routes.setPattern("PATCH /api/v1/policies/:id", async (ctx) => {
        ...
        if (typeof body.title === "string")
            contentPatch.title = body.title;
        if (typeof body.trigger === "string")
            contentPatch.trigger = body.trigger;
        if (typeof body.procedure === "string")
            contentPatch.procedure = body.procedure;
        if (typeof body.verification === "string")
            contentPatch.verification = body.verification;
        if (typeof body.boundary === "string")
            contentPatch.boundary = body.boundary;
        const hasContent = Object.keys(contentPatch).length > 0;
        const hasStatus = body.status !== undefined;
        if (!hasStatus && !hasContent) {
            writeError(ctx, 400, "invalid_argument", "provide at least one of status / title / trigger / procedure / verification / boundary");
            return;
        }
        if (hasStatus && !isValidPolicyStatus(body.status)) {
            writeError(ctx, 400, "invalid_argument", "status must be one of: candidate, active, archived");
            return;
        }
        ...
        if (hasStatus) {
            updated = await deps.core.setPolicyStatus(id, body.status);
            if (!updated) {
                writeError(ctx, 404, "not_found", `policy not found: ${id}`);
                return;
            }
    ...
    routes.setPattern("POST /api/v1/policies/:id/guidance", async (ctx) => {
        ...
        if (pref.length === 0 && avoid.length === 0) {
            writeError(ctx, 400, "invalid_argument", "provide at least one preference or antiPattern string");
            return;
        }
    ...
    routes.setPattern("GET /api/v1/world-models/:id/usage", async (ctx) => {
        ...
        const policies = await Promise.all(wm.policyIds.map(async (pid) => {
    ...
    routes.setPattern("PATCH /api/v1/world-models/:id", async (ctx) => {
        ...
        if (Object.keys(patch).length === 0) {
            writeError(ctx, 400, "invalid_argument", "provide at least one of title / body / status");
            return;
        }
...
function isValidPolicyStatus(v) {
    return v === "candidate" || v === "active" || v === "archived";
}
function normalizeGuidanceList(raw) {
