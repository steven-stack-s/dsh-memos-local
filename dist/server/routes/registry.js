...
export class Routes {
    ...
    matchPattern(method, pathname) {
        for (const pat of this.patterns) {
            if (pat.method !== method)
                continue;
    ...
    allPaths() {
        ...
        for (const p of this.patterns)
            out.push(p.regex.source);
        return out;
    }
    /** Patterns that match a pathname regardless of method (for 405). */
    pathMatches(pathname) {
        for (const k of this.exact.keys()) {
            if (k.split(" ")[1] === pathname)
                return true;
        }
        for (const p of this.patterns) {
            if (p.regex.test(pathname))
                return true;
        }
        return false;
    }
}
export function buildRoutes(deps, options) {
...
export function parseJson(ctx) {
    try {
        return parseJsonBody(ctx.body);
    }
    catch (err) {
...
export function writeError(ctx, status, code, message) {
